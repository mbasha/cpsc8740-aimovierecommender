...
func GetUser(username string) (*User, bool) {
	...
	err := DB.QueryRow(
		`SELECT character, is_new FROM users WHERE username = $1`,
		username,
...
func GetRatings(username string) map[string]float64 {
	rows, err := DB.Query(
		`SELECT movie_id, rating FROM ratings WHERE username = $1`,
		username,
	)
	if err != nil {
...
func SaveRatings(username string, ratings map[string]float64) {
	for movieIDStr, rating := range ratings {
		var movieID int
		json.Unmarshal([]byte(movieIDStr), &movieID)
		_, err := DB.Exec(
...
func SaveRecommendations(username string, recs []Recommendation) {
	_, err := DB.Exec(
		`DELETE FROM recommendations WHERE username = $1`, username,
	)
	if err != nil {
...
func RemoveFromWatchlist(username string, movieID int) {
	_, err := DB.Exec(
		`DELETE FROM watchlist WHERE username = $1 AND movie_id = $2`,
		username, movieID,
	)
	if err != nil {
...
func RemoveFromHidden(username string, movieID int) {
	_, err := DB.Exec(
		`DELETE FROM hidden WHERE username = $1 AND movie_id = $2`,
		username, movieID,
	)
	if err != nil {
